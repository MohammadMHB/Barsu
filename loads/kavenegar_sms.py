...
def send_sms_to_driver(phone_number, message):
    ...
    if not line_number:
        print("⚠️ SMS_IR_LINE_NUMBER تنظیم نشده، تلاش بدون شماره خط")
...
def notify_drivers_new_load(load, drivers):
    print("="*50)
    print(f"🔍 notify_drivers_new_load فراخوانی شد")
    print(f"🔍 تعداد رانندگان: {len(drivers)}")
    
    for driver in drivers:
        print(f"   - {driver.user.username}: {driver.user.phone_number}")
